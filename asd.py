...
API_URL = "http://local.adspower.net:50325/api/v1"
...
async def setup_browser(ws_endpoint, proxy=None):
    max_attempts = 3
    for attempt in range(max_attempts):
        try:
            ...
            if proxy:
                proxy_auth = proxy.split("://")[1].split("@")[0]
                proxy_host = proxy.split("@")[1]
                print(f"{Fore.CYAN}Настраиваем прокси...{Style.RESET_ALL}")
                await page.authenticate({"username": proxy_auth.split(":")[0], "password": proxy_auth.split(":")[1]})
                print(f"{Fore.GREEN}✓ Прокси настроен: {proxy}{Style.RESET_ALL}")
...
async def check_and_start_prompts(page, profile_id):
    try:
        print(f"{Fore.CYAN}Переход на https://klokapp.ai?referral_code=TMJMN9HN...{Style.RESET_ALL}")
        await page.goto("https://klokapp.ai?referral_code=TMJMN9HN", {"waitUntil": "networkidle2"})
        ...
        try:
            ...
            if google_button:
                ...
                print(f"{Fore.CYAN}Кнопка Continue with Google нажата, ждём 15 секунд...{Style.RESET_ALL}")
        ...
        print(f"{Fore.RED}✗ Ошибка проверки авторизации: {e}{Style.RESET_ALL}")
        return False
...
async def send_prompts(page, profile_id):
    try:
        ...
        print(f"{Fore.CYAN}Проверяем количество оставшихся промптов...{Style.RESET_ALL}")
        ...
        if not used_prompts_match:
            print(f"{Fore.RED}✗ Не удалось извлечь числа из текста счётчика: '{counter_text}'{Style.RESET_ALL}")
            ...
            if alt_match:
                ...
                raise Exception("Не удалось извлечь количество использованных промптов из текста")
        else:
...
async def main():
    total_profiles = min(15, len(profile_ids))

    for idx, profile_id in enumerate(profile_ids[:total_profiles]):
        proxy = proxies[idx] if idx < len(proxies) else random.choice(proxies)

        print(f"{Fore.MAGENTA}=== Работа началась с профилем [{idx}] {profile_id} ==={Style.RESET_ALL}")
        print(f"{Fore.MAGENTA}Используем прокси: {proxy}{Style.RESET_ALL}")
        ...
        browser, page = await setup_browser(ws_endpoint, proxy)
